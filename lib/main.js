...
export default class Flint {
  ...
  handleConnection(path, connection) {
    ...
    connection.onMessage('compile:error', message => {
      ...
      if (!error.loc || !errorMessage) {
        console.debug('[Flint] error message by compiler is invalid', error)
      }
      this.linter.setError(connection, error.fileName, {
        type: 'Error',
        text: errorMessage || 'Unknown message, check console for more info',
        filePath: error.fileName,
        range: error.loc ? [[error.loc.line - 1, error.loc.column - 1], [error.loc.line - 1, error.loc.column]] : [[0, 0], [0, 1]]
      })
    })
