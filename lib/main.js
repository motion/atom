...
export class Motion {
  ...
  constructor() {
    ...
    this.connections.onShouldProvideEditor(e => {
      e.editor = this.editors.get(e.textEditor)
    })
  }
  activate() {
  ...
  async consumeStatusbar(statusBar: Object): Promise {
  ...
  provideAutocomplete(): Autocomplete {
    ...
    autocomplete.onShouldProvideEditor(e => {
      e.editor = this.editors.get(e.textEditor)
    })
    this.subscriptions.add(autocomplete)
    return autocomplete
  }
  provideDeclarations(): Declarations {
    const declarations = new Declarations()
    declarations.onShouldProvideEditor(e => {
      e.editor = this.editors.get(e.textEditor)
    })
