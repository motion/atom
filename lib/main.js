...
import {fileExists, getMessage, showError, realPath, getGrammar} from './helpers'
...
export default class Flint {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.workspace.observeTextEditors(textEditor => {
      let flintGrammar = NaN
      const subscription = this.subscriptions.add(textEditor.onDidStopChanging(() => {
        let isFlintGrammar = textEditor.getText().indexOf('view ') !== -1

        if (flintGrammar !== isFlintGrammar) {
          textEditor.setGrammar(getGrammar(isFlintGrammar))
          flintGrammar = isFlintGrammar
        }
      }))
  ...
  handleConnection(path, connection) {
    ...
    connection.onMessage('file:meta', message => {
      const textEditor = this.getEditorByPath(message.file)
      if (textEditor !== null) {
