  ...
  constructor(editor: TextEditor) {
    ...
    this.subscriptions.add(editor.onDidStopChanging(() => {
      queue = queue.then(async function() {
        ...
        let cursorBufferPosition = editor.getCursorBufferPosition()
        editor.scan(MAGICAL_REGEX, function({ matchText, replace, range }) {
          ...
          if (range.containsPoint(cursorBufferPosition)) {
            cursorBufferPosition = editor.getCursorBufferPosition()
            cursorBufferPosition.column -= 1
