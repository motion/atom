...
import {aggressiveDebounce, rangeFromCompilerLocation} from './helpers'
...
export default class Editor {
  ...
  updateMeta(meta) {
    ...
    for (const key in meta) {
      ...
      value.name = key
      value.position = rangeFromCompilerLocation(value.location)
      views.push(value)
    }
    this.views = views
  }
  getViewAtPoint(position) {
    let i = 0
    let view
    while (view = this.views[i++]) {
      if (view.position.containsPoint(position)) {
        return view
      }
    }
    return null
